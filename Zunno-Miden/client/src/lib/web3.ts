...
import { getMidenContract } from './midenAdapter';
...
export async function getContractNew() {
  try {
    if (process.env.NEXT_PUBLIC_MIDEN_ENABLED === 'true') {
      console.log('Using Miden adapter for contract calls');
      return getMidenContract();
    }
