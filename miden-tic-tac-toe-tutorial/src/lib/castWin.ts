...
import castWinNoteCode from "./notes/cast_win_note";
...
export async function castWin(
  ...
  const noteScript = assembler.compileNoteScript(castWinNoteCode);

  console.log("Inputs values", winningLine);

  const noteInputs = new NoteInputs(
    new FeltArray([
      new Felt(BigInt(0)),
      new Felt(BigInt(winningLine[2])),
      new Felt(BigInt(winningLine[1])),
      new Felt(BigInt(winningLine[0])),
