...
import makeWinMoveCode from "./notes/make_win_move_code";
...
// lib/makeMove.ts
export async function makeWinMove(
  nonce: number,
  fieldIndex: number,
  ...
  const noteScript = assembler.compileNoteScript(makeWinMoveCode);

  const noteInputs = new NoteInputs(
    new FeltArray([
      new Felt(BigInt(winningFieldIndex2)),
      new Felt(BigInt(winningFieldIndex1)),
      new Felt(BigInt(fieldIndex)),
